...
angular.module('chApp.patients.services').factory('patientMedicationsService', [
    '$http', '$state', '$cookies', "appConfig",
    function ($http, $state, $cookies, appConfig) {
    var service = {};
    var oElements = {};
    var _apiName = "/patient-medications/";

    service.getMedications = function (patientId) {
        ...
        function getFailed(error) {
            alert('XHR Failed for getPatientMedications.' + error.data);
        }
    };
    service.getMedication = function (medicationId) {
        return $http.get(appConfig.API_HOST + _apiName + medicationId)
                    .then(getComplete)
                    .catch(getFailed);
    ...
    service.insertMedication = function (patientId, medication) {
        var promise = null;
        if (!patientId) {
            ...
            promise = $http.post(appConfig.API_HOST + _apiName, medication);
        }

        return promise;
    }

    service.updateMedication = function (patientId, medication) {
        var promise = null;
        if (!patientId) {
            var deferred = $q.defer();
            setTimeout(function () {
            ...
            promise = $http.put(appConfig.API_HOST + _apiName + medication.id, medication);
        }
    ...
    service.getDrugById = function(drugId){
        return $http.get(appConfig.API_HOST + "/master/drug/" + drugId)
            .then(getComplete)
            .catch(getFailed);
    ...
    service.initWidgets = function (practiceId, selector, oTable, insertModal, editModal, rowDataChangedCallback, selectIcdCallback, selectDrugCallback) {
        insertModal = service.initInsertModal(practiceId, selector, insertModal, selectIcdCallback, selectDrugCallback);
        editModal = service.initEditModal(practiceId, selector, editModal, selectIcdCallback, selectDrugCallback);
    ...
    service.initTable = function (selector, oTable, insertModal, editModal, rowDataChangedCallback) {
        ...
        oTable = $(selector).find(".medicationList.dataTable");
        ...
        function fnFormatDetails(aData) {
            ...
            $('dl:eq(0)', oRow).append('<dt style="width:80px">规格</dt><dd style="margin-left:100px">' + (data.form || '') + '; ' + (data.strength || '') + (data.strengthUnit || '') + '</dd>');
            $('dl:eq(0)', oRow).append('<dt style="width:80px">用药频率</dt><dd style="margin-left:100px">' + (data.frequency || '') + '</dd>');
            $('dl:eq(1)', oRow).append('<dt style="width:80px">用药原因</dt><dd style="margin-left:100px">' + (data.problemName || '') + '</dd>');
            if (data.currentStatus != '服用中'){
                $('dl:eq(1)', oRow).append('<dt style="width:80px">停药原因</dt><dd style="margin-left:100px">' + (data.stopReason || '') + '</dd>');
            }
            $(oOut).append(oRow);
    ...
    service.initInsertModal = function (practiceId, selector, insertModal, selectIcdCallback, selectDrugCallback) {
        var token = $cookies.get(appConfig.CH_AU_T_NAME);
        var tokenName = appConfig.CH_AU_T_NAME;

        var oModal = $(insertModal || ".insertMedicationModal", selector);
        ...
        var drugSearch = oModal.find('.drugSearch');
        $(drugSearch).select2("destroy");
        $(drugSearch).select2({
            formatResult: format,
            formatSelection: format,
            minimumInputLength: 1,
            ...
            ajax: {
                ...
                url: appConfig.API_HOST + "/master/drug/query/options",
                dataType: 'json',
                delay: 250,
                data: function (params) {
                    return {
        ...
        $(drugSearch).on("select2-selected", function (e) {
            var drugId = e.choice.id;
            service.getDrugById(drugId).then(function (result) {
                if (typeof selectDrugCallback === "function") {
                    selectDrugCallback(result);
                }
            });
        });
    ...
    service.initEditModal = function (practiceId, selector, editModal, selectIcdCallback, selectDrugCallback) {
        var oModal = $(editModal || ".editMedicationModal", selector);
